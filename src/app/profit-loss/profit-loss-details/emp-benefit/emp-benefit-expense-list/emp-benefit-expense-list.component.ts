...
@Component({
  selector: 'app-emp-benefit-expense-list',
  standalone: false,
  templateUrl: './emp-benefit-expense-list.component.html',
  styleUrl: './emp-benefit-expense-list.component.scss'
})
export class EmpBenefitExpenseListComponent {
 data:any[]=[];
  isLoading = true;
  ...
  private subscription:Subscription = new Subscription();
   ...
   ngOnInit()  {
    this.stateDataService.stateDataSubject.subscribe((data) => {   
      if (data.event == 'empbedit'  && data.valid && data.value) {      
        this.updateRowData(data.value);
        this.notifyBarService.showsnackbar(data.msg);
        this.stateDataService.stateDataSubject.next({});
      } else if (data.event == 'empbadd' && data.valid && data.value) {
        this.addRowData(data.value);
        this.notifyBarService.showsnackbar(data.msg);
        this.stateDataService.stateDataSubject.next({});
      } else if(data.event == 'empbdelete' && data.valid && data.value){
    ...
    this.subscription=  this.sessionService.profileLossScopeSubject$.pipe(take(1),untilDestroyed(this)).subscribe((responseData:any)=>{
      if(responseData.length>0){
        this.profitLossService.getProfitLossByScopeId({id:responseData.find((x:any)=> x.key== ProfitLossScope.EMPLOYEE_BENEFIT_EXPENSES)?.id }, '')
