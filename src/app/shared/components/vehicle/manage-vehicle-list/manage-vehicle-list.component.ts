...
export class ManageVehicleListComponent {
  ...
  displayedColumns: string[] = ['serial', 'projectshortname', 'name', 'vehiclenum', 'fixedkm', 'fixedbillamt', 'extraamtabovefixkm', 'log', 'docs'];
  ...
  ngOnInit() {
    ...
    if (PermissionGuids.vehicle) {
      this.commonService.getPermissionsForCurrentPage(PermissionGuids.vehicle).then((permissions: any) => {
  ...
  viewdocs(data: any) {
    this.defaultdialogoptions.data = {
      pageGuid: PermissionGuids.vehicle,
      element: { id: data }
    };
    this.defaultdialogoptions.minWidth = '75vw';
