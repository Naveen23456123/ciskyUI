...
@Component({
  selector: 'app-manage-profit-loss-sheet',
  standalone: false,
  templateUrl: './manage-profit-loss-sheet.component.html',
  styleUrl: './manage-profit-loss-sheet.component.scss'
})
export class ManageProfitLossSheetComponent {
 ...
 public data: any;
   ...
   constructor(@Inject(MAT_DIALOG_DATA) data: any,
     @Optional() private dialogRef: MatDialogRef<ManageProfitLossSheetComponent>, private formbuilder: FormBuilder,
   ...
   submit(){  
     if (this.isEdit) {
       this.profitLossService.updateProfitLossSymbol(this.profitLossForm.value, '')
       ...
       this.profitLossService.createProfitLossSheet(this.profitLossForm.value, '')
         .pipe(finalize(() => { this.isLoading = false; })).subscribe({
           next:(response: any) => {
             if (response && response.success)  {
   ...
   delete() {
       this.profitLossService.deleteProfitLossSheet({id:this.profitLossForm.value.id}, '')
        .pipe(finalize(() => { this.isLoading = false; })).subscribe({
         next:(response: any) => {
           if (response && response.success) 
             this.dialogRef.close({ value: this.profitLossForm.value, valid: true });
