...
public data: any;
 ...
 ngOnDestroy(){}
      ...
      if(response && response.projectId){
        ...
        if (this.isEdit) {
          ...
          let formData= {
            ...
            scopes:[this.dtForm.value]
          }
          this.dutyTravelService.createBoqDutyTravel(formData, '')
            .pipe(finalize(() => { this.isLoading = false;this.isBtnClicked=false })).subscribe({
              next:(response: any) => {
              if (response && response.success) {
                this.dtForm.addControl('totalamount', this.formbuilder.control(response.data.scopes[0].totalamount));
                this.dtForm.controls["id"].setValue(response.data.scopes[0].id);
                this.dialogRef.close({ value: this.dtForm.value, valid: true });
              } else {
  ...
  desc_submit(){
    this.isBtnClicked=true;
    this.dutyTravelService.UpdateBoqDutyTravelDescription(this.headingForm.value, '')
