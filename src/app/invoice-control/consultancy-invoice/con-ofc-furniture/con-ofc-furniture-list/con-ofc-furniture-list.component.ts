...
export class ConOfcFurnitureListComponent {
  ...
  dataColumn: string[] = ['serial', 'desc', 'month', 'rate', 'totalamt', 'prev_month', 'prev_amt', 'curr_month', 'curr_amt', 'comlt_month', 'comlt_amt', 'rem_month', 'rem_amt'];
  ...
  ngOnInit() {
    ...
    this.sessionService.invoiceEntitySubject$.pipe(take(1)).subscribe((invEntity: any) => {
      if (invEntity && invEntity.invoiceId) {
        this.invoiceService.getOfcFurnitureScopeByProjectId({ id: invEntity.invoiceId, projectid: invEntity.projectId }, '')
