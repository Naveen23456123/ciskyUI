...
export class ConStaffListComponent {
  ...
  amount: any | null;
  headerColumn: string[] = ['no', 'desgth', 'nameth', 'ctrtamt', 'previousinvc', 'currentinvc', 'cumulativeinvc', 'reminvc'];
  dataColumn: string[] = ['serial', 'desg', 'name', 'contract_rate', 'const', 'main', 'totalconstmain', 'cont_amt', 'prev_month', 'prev_amt', 'curr_month', 'curr_amt', 'comlt_month', 'comlt_amt', 'rem_month', 'rem_amt'];
    ...
    this.subscription = this.sessionService.invoiceEntitySubject$.pipe(take(1)).subscribe((invEntity: any) => {
      if (invEntity && invEntity.projectId) {
        forkJoin({
          ...
          boqstaffAPI: this.invoiceService.getStaffScopeByProjectId({ id: invEntity.invoiceId, projectid: invEntity.projectId }, '')
          ...
          if (response.staffTypeAPI && response.staffTypeAPI.success) {
            ...
            if (response.staffTypeAPI && response.boqstaffAPI.success) {
              this.totalData = response.boqstaffAPI.data.map((data: any) => ({
                ...
                name: data.name?.trim() || BOQ_TBN.TBN,
                rate: data.rate,
  ...
  updateRowData(newdata: any) {
    ...
    if (prfSelected.name === StaffType.KEY_PROFESSIONAL) {
      element = this.dataSource.data.find((x: any) => x.id == data.boqid);
    }
    else if (prfSelected.name === StaffType.SUPPORT_STAFF) {
      element = this.ssdataSource.data.find((x: any) => x.id == data.boqid);
    }
    else {
      element = this.spsdataSource.data.find((x: any) => x.id == data.boqid);
    }

    if (element) {
      if (data.isEditing)
        element.name = data.employeename;
  ...
  deleteRow(newdata: any) {
    ...
    const prfSelected = newdata.professionalData.find((option: any) => option.id === data.professionalid);
    if (prfSelected.name === StaffType.KEY_PROFESSIONAL) {
      const index = this.dataSource.data.findIndex((x: any) => x.id == data.scopeid);
