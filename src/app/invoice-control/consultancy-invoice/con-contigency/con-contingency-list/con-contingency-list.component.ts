...
export class ConContingencyListComponent {
  dataColumn: string[] = ['serial', 'desc', 'previous', 'current', 'commu', 'remaining'];
  ...
  ngOnInit() {
    ...
    this.sessionService.invoiceEntitySubject$.pipe(take(1)).subscribe((invEntity: any) => {
      if (invEntity && invEntity.invoiceId) {
        this.invoiceService.getContingencyScopeByProjectId({ id: invEntity.invoiceId, projectid: invEntity.projectId }, '')
