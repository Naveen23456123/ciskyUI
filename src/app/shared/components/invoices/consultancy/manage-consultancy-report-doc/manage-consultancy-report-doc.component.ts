...
export class ManageConsultancyReportDocComponent {
    ...
    (this.rdForm.get('controls') as FormArray).controls.forEach((group: AbstractControl, index: number) => {
      ...
      if (quantityControl) {
        quantityControl.statusChanges.subscribe(value => {
          setTimeout(() => {
            group.get('currentbillamount')?.setValue(quantityControl.value * (this.boqList.find(x => x.id == group.get('boqid')?.value).ratepercopy));
          });
        });
      }
    });
  ...
  submit() {
    ...
    this.sessionService.invoiceEntitySubject$.pipe(take(1), untilDestroyed(this)).subscribe((response: any) => {
      if (response && response.invoiceId) {
        ...
        if (this.isEdit) {
          this.invoiceService.upsertReportDocInvoiceScope(this.rdForm.get('controls')?.value, '')
  ...
  delete() {
    this.invoiceService.deleteInvoiceReportDocScope(this.data.element, '')
