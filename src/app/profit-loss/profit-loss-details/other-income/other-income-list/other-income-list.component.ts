...
@Component({
  ...
  styleUrl: './other-income-list.component.scss'
})
export class OtherIncomeListComponent {
  data:any[]=[];
  isLoading = true;
  ...
  private subscription:Subscription = new Subscription();
   ...
   ngOnInit()  {
    this.stateDataService.stateDataSubject.subscribe((data) => {   
      if (data.event == 'oiedit'  && data.valid && data.value) {      
        this.updateRowData(data.value);
        this.notifyBarService.showsnackbar(data.msg);
        this.stateDataService.stateDataSubject.next({});
      } else if (data.event == 'oiadd' && data.valid && data.value) {
        this.addRowData(data.value);
        this.notifyBarService.showsnackbar(data.msg);
        this.stateDataService.stateDataSubject.next({});
      } else if(data.event == 'oidelete' && data.valid && data.value){
    ...
    this.subscription=  this.sessionService.profileLossScopeSubject$.pipe(take(1),untilDestroyed(this)).subscribe((responseData:any)=>{
      if(responseData.length>0){
        this.profitLossService.getProfitLossByScopeId({id:responseData.find((x:any)=> x.key== ProfitLossScope.OTHER_INCOME)?.id }, '')
