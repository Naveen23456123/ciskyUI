...
import { BOQ_TBN, StaffType } from '@app/shared/models/constant.config';
...
export class BoqStaffListComponent {
  ...
  isLoading = true;
  kpsDataColumn: string[] = ['serial', 'position', 'name', 'constperiod', 'omperiod', 'manmonth', 'rate', 'amount'];
  spsdataColumn: string[] = ['serial', 'position', 'name', 'constperiod', 'omperiod', 'manmonth', 'rate', 'amount'];
  ssdataColumn: string[] = ['serial', 'position', 'name', 'constperiod', 'omperiod', 'manmonth', 'rate', 'amount'];
  ...
  ngOnInit() {
    ...
    if (this.pagePermissions?.canUpdate || this.pagePermissions?.canDelete) {
      this.kpsDataColumn.push('action');
      this.spsdataColumn.push('action');
      this.ssdataColumn.push('action');
          ...
          if (response.staffTypeAPI && response.staffTypeAPI.success) {
            ...
            if (response.staffTypeAPI && response.boqstaffAPI.success) {
              ...
              if (this.totalData && this.totalData.length > 0) {
                ...
                this.dataSource = new MatTableDataSource(this.kpsData.map((item: any) => ({
                  ...item,
                  employeename: item.employeename?.trim() || BOQ_TBN.TBN
                })));

                this.ssData = this.totalData.filter((x: any) => x.professionalid == ssItem.id) ?? [];                
                this.ssdataSource = new MatTableDataSource(this.ssData.map((item: any) => ({
                  ...item,
                  employeename: item.employeename?.trim() || BOQ_TBN.TBN
                })));

                this.spsData = this.totalData.filter((x: any) => x.professionalid == spsItem.id) ?? [];
                this.spsdataSource = new MatTableDataSource(this.spsData.map((item: any) => ({
                  ...item,
                  employeename: item.employeename?.trim() || BOQ_TBN.TBN
                })));
  ...
  updateRowData(newdata: any) {
    ...
    if (element) {
      ...
      element.designationid = data.designationid,
        ...
        element.employeename = data.employeename?.trim() || BOQ_TBN.TBN,
  ...
  addRowData(newdata: any) {
    ...
    const data1: any = {
      ...
      employeename: data.employeename?.trim() || BOQ_TBN.TBN,
