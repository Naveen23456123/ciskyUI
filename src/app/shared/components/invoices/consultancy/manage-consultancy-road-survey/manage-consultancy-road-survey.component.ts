...
export class ManageConsultancyRoadSurveyComponent {
    ...
    (this.rsForm.get('controls') as FormArray).controls.forEach((group: AbstractControl, index: number) => {
      ...
      if (quantityControl) {
        quantityControl.statusChanges.subscribe(value => {
          setTimeout(() => {
            group.get('currentbillamount')?.setValue(quantityControl.value * (this.boqList.find(x => x.id == group.get('boqid')?.value).ratepersurvey));
          });
        });
      }
    });
  ...
  submit() {
    ...
    this.sessionService.invoiceEntitySubject$.pipe(take(1), untilDestroyed(this)).subscribe((response: any) => {
      if (response && response.invoiceId) {
        ...
        if (this.isEdit) {
          this.invoiceService.upsertRoadSurveyInvoiceScope(this.rsForm.get('controls')?.value, '')
  ...
  delete() {
    this.invoiceService.deleteInvoiceRoadSurveyScope(this.data.element, '')
