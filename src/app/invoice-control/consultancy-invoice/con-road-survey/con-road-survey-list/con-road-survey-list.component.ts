...
export class ConRoadSurveyListComponent {
  ...
  dataColumn: string[] = ['serial', 'desc', 'km', 'rateperkm', 'survery_conducted', 'total_amt', 'prev_km', 'prev_amt', 'curr_km', 'curr_amt', 'comlt_km', 'comlt_amt', 'rem_km', 'rem_amt'];
  ...
  ngOnInit() {
    ...
    this.sessionService.invoiceEntitySubject$.pipe(take(1)).subscribe((invEntity: any) => {
      if (invEntity && invEntity.invoiceId) {
        this.invoiceService.getRoadSurveyScopeByProjectId({ id: invEntity.invoiceId, projectid: invEntity.projectId }, '')
