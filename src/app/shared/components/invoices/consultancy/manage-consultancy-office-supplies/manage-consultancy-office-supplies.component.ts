...
export class ManageConsultancyOfficeSuppliesComponent {
    ...
    if (!this.deleteos) {
      this.sessionService.invoiceEntitySubject$.pipe(take(1)).subscribe((projectEntity: any) => {
        if (projectEntity && projectEntity.projectId) {
          if (!this.isEdit) {
            this.boqService.getBoqOfficeSupplyListForInsertByProjectId({ invid: projectEntity.invoiceId, id: projectEntity.projectId }, '')
  ...
  subscribeChange() {
    (this.osForm.get('controls') as FormArray).controls.forEach((group: AbstractControl, index: number) => {
  ...
  submit() {
    ...
    this.sessionService.invoiceEntitySubject$.pipe(take(1), untilDestroyed(this)).subscribe((response: any) => {
      if (response && response.invoiceId) {
        ...
        if (this.isEdit) {
          this.invoiceService.upsertOfcSupplyInvoiceScope(this.osForm.get('controls')?.value, '')
  ...
  delete() {
    this.invoiceService.deleteInvoiceOfcSupplyScope(this.data.element, '')
