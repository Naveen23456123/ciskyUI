...
export class ConReportDocListComponent {
  ...
  dataColumn: string[] = ['serial', 'desc', 'no_ofreport', 'copies_per_report', 'total_copy', 'rate_per_copy', 'total_amt', 'prev_month', 'prev_amt', 'curr_month', 'curr_amt', 'comlt_month', 'comlt_amt', 'rem_month', 'rem_amt'];
  ...
  ngOnInit() {
    ...
    this.sessionService.invoiceEntitySubject$.pipe(take(1)).subscribe((invEntity: any) => {
      if (invEntity && invEntity.invoiceId) {
        this.invoiceService.getReportDocScopeByProjectId({ id: invEntity.invoiceId, projectid: invEntity.projectId }, '')
  ...
  bindBilling(data: any) {
    if (data) {
      data.contractamount = data.numberofreport * data.numberofcopiesperreport * data.ratepercopy,
        data.previousbill = data.previousbillmonths * data.ratepercopy,
        data.currentbill = data.currentbillmonths * data.ratepercopy,
        data.commulativemonth = data.previousbillmonths + data.currentbillmonths,
        data.commulativeamount = (data.previousbillmonths + data.currentbillmonths) * data.ratepercopy,
        data.remainingmonth = (data.numberofreport * data.numberofcopiesperreport) - (data.previousbillmonths + data.currentbillmonths),
        data.remainingamount = ((data.numberofreport * data.numberofcopiesperreport) * data.ratepercopy) - ((data.previousbillmonths + data.currentbillmonths) * data.ratepercopy)
    }
  }
