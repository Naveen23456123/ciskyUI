...
readonly dialog = inject(MatDialog);
  ...
  openDialog(): void {
    ...
    const dialogRef = this.dialog.open(ManageApprovalComponent, config);
    dialogRef.afterClosed().subscribe((data) => {
      if (data && data.valid) {
        let navigationExtras: NavigationExtras = {
          relativeTo: this.route,
          state: { value: data.value, event: 'apradd', valid: true, msg: 'The Approval settings created successfully.' }
        };
