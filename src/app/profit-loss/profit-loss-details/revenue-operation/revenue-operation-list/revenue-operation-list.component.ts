...
@Component({
  selector: 'app-revenue-operation-list',
  standalone: false,
  templateUrl: './revenue-operation-list.component.html',
  styleUrl: './revenue-operation-list.component.scss'
})
export class RevenueOperationListComponent {
  data:any[]=[];
  isLoading = true;
  ...
  private subscription:Subscription = new Subscription();
   ...
   ngOnInit()  {
    this.stateDataService.stateDataSubject.subscribe((data) => {   
      if (data.event == 'prvedit'  && data.valid && data.value) {      
        this.updateRowData(data.value);
        this.notifyBarService.showsnackbar(data.msg);
        this.stateDataService.stateDataSubject.next({});
      } else if (data.event == 'prvadd' && data.valid && data.value) {
        this.addRowData(data.value);
        this.notifyBarService.showsnackbar(data.msg);
        this.stateDataService.stateDataSubject.next({});
      } else if(data.event == 'prvdelete' && data.valid && data.value){
    ...
    this.subscription=  this.sessionService.profileLossScopeSubject$.pipe(take(1),untilDestroyed(this)).subscribe((responseData:any)=>{
      if(responseData.length>0){
        this.profitLossService.getProfitLossByScopeId({id:responseData.find((x:any)=> x.key== ProfitLossScope.REVENUE_FROM_OPERATION)?.id }, '')
