...
import { DialogOperation, LetterEntity, LetterType, PermissionGuids } from '@app/shared/models/constant.config';
...
export class ManageLettersListComponent {
  lettersList: any[] = [];
  isLoading = true;
  displayedColumns: string[] = ['serial', 'letterno', 'lettertype', 'subject', 'letterdate', 'status'];
  ...
  ngOnInit() {
    this.isLoading = true;
    if (PermissionGuids.letters) {
      this.commonService.getPermissionsForCurrentPage(PermissionGuids.letters).then((permissions: any) => {
        if (permissions) {
