...
@Component({
  selector: 'app-supervision-create',
  standalone: false,
  templateUrl: './supervision-create.component.html',
  styleUrl: './supervision-create.component.scss'
})
export class SupervisionCreateComponent {
readonly dialog = inject(MatDialog);
  ...
  openDialog(): void {
    ...
    const dialogRef = this.dialog.open(ManageSupervisionComponent, config);
    dialogRef.afterClosed().subscribe((data) => {
      if (data && data.valid) {
        let navigationExtras: NavigationExtras = {
          relativeTo: this.route,
          state: { value: data.value, event: 'sup-projectadd', valid: true, msg: 'The Project Created successfully.' }
        };
