...
@Component({
  selector: 'app-delete-con-ofc-rent',
  standalone: false,
  templateUrl: './delete-con-ofc-rent.component.html',
  styleUrl: './delete-con-ofc-rent.component.scss'
})
export class DeleteConOfcRentComponent {
ngOnInit(): void {
    this.openDialog();
  }
  restrictDialog() {
  ...
  openDialog() {
    ...
    const dialogRef = this.dialog.open(ManageConsultancyOfficeRentComponent, this.defaultdialogOptionConfig);
    dialogRef.afterClosed().subscribe((data) => {
      if (data.valid) {
        let navigationExtras: NavigationExtras = {
          relativeTo: this.route,
          state: { value: data.value, event: 'conordelete', valid: true, msg: 'The Office Rent invoice details removed successfully.' }
        };
