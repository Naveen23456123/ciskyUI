...
@Component({
  selector: 'app-delete-con-contingency',
  standalone: false,
  templateUrl: './delete-con-contingency.component.html',
  styleUrl: './delete-con-contingency.component.scss'
})
export class DeleteConContingencyComponent {
ngOnInit(): void {
    this.openDialog();
  }
  restrictDialog() {
  ...
  openDialog() {
    ...
    const dialogRef = this.dialog.open(ManageConsultancyContingenciesComponent, this.defaultdialogOptionConfig);
    dialogRef.afterClosed().subscribe((data) => {
      if (data.valid) {
        let navigationExtras: NavigationExtras = {
          relativeTo: this.route,
          state: { value: data.value, event: 'concontdelete', valid: true, msg: 'The Contingency invoice details removed successfully.' }
        };
