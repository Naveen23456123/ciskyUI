...
@Component({
  selector: 'app-delete-con-ofc-supply',
  standalone: false,
  templateUrl: './delete-con-ofc-supply.component.html',
  styleUrl: './delete-con-ofc-supply.component.scss'
})
export class DeleteConOfcSupplyComponent {
...
ngOnInit(): void {
  ...
  openDialog() {
    ...
    const dialogRef = this.dialog.open(ManageConsultancyOfficeSuppliesComponent, this.defaultdialogOptionConfig);
    dialogRef.afterClosed().subscribe((data) => {
      if (data.valid) {
        let navigationExtras: NavigationExtras = {
          relativeTo: this.route,
          state: { value: data.value, event: 'conosdelete', valid: true, msg: 'The Office  Supply invoice details removed successfully.' }
        };
