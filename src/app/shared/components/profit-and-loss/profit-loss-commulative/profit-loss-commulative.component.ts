...
ngOnInit(){
  ...
  buildGroupedRows(data: any[]): Row[] {
    ...
    this.keyToGroup.forEach((groupName, index) => {
      ...
      data.filter(d => d.isadmin === groupName.isadmin && d.ispersonal === groupName.ispersonal).forEach((d,index) => {     
        if(d.type!=='totalfooter'){           
          result.push({ ...d,srno:index+1,
             type: 'data'});
          }     
      });
      ...
      result.push({
        ...
        name:'Total',
        id:groupName.id,
        actualamount: this.commonService.roundValue(this.getCurrentAmount(data.filter(d => d.isadmin === groupName.isadmin && d.ispersonal === groupName.ispersonal))),        
