...
export class ConTransportationListComponent {
  ...
  tpdataColumn: string[] = ['serial', 'desc', 'contract_rate', 'const', 'main', 'totalconstmain', 'cont_amt', 'prev_month', 'prev_amt', 'curr_month', 'curr_amt', 'comlt_month', 'comlt_amt', 'rem_month', 'rem_amt'];
  ...
  ngOnInit() {
    ...
    this.sessionService.invoiceEntitySubject$.pipe(take(1)).subscribe((projectEntity: any) => {
      if (projectEntity && projectEntity.invoiceId) {
        this.invoiceService.getTransportScopeByProjectId({ id: projectEntity.invoiceId, projectid: projectEntity.projectId }, '')
