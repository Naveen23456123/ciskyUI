...
export class ConOfcFurnitureListComponent {
  ...
  dataColumn: string[] =['serial','desc','unit','qty','rate','totalamt','prev_month','prev_amt','curr_month','curr_amt','comlt_month','comlt_amt','rem_month','rem_amt','action'];
   ...
   ngOnInit()  {
    ...
    this.sessionService.invoiceEntitySubject$.pipe(take(1)).subscribe((invEntity:any)=>{
      if(invEntity && invEntity.invoiceId){
        this.invoiceService.getConsultantOfficeFurnitureListByProjectId({id:invEntity.invoiceId }, '')
  ...
  getattributes(data:any){
    return {
      contractamount:data.quantity*data.rateperunit,
      previousbill:data.previousbillmonths*data.rateperunit,
      currentbill:data.currentbillmonths*data.rateperunit,
      commulativemonth:data.previousbillmonths+data.currentbillmonths,
      commulativeamount:(data.previousbillmonths+data.currentbillmonths)*data.rateperunit,
      remainingmonth:(data.quantity)-(data.previousbillmonths+data.currentbillmonths),
      remainingamount:((data.quantity)*data.rateperunit)-((data.previousbillmonths+data.currentbillmonths)*data.rateperunit)
    }
  }
    ...
    updateRowData(data: any) {
      ...
      element.rateperunit=data.rateperunit,
